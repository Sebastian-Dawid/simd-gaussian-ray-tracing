...
f32 transmittance(const vec4f_t o, const vec4f_t n, const f32 s, const gaussians_t &gaussians)
{
    ...
    for (const gaussian_t &g_q : gaussians.gaussians)
    {
        ...
        const f32 c_bar = g_q.magnitude * EXP( -((g_q.mu - o).dot(g_q.mu - o) - std::pow(mu_bar, 2.f))/(2*std::pow(g_q.sigma, 2.f)) );
        T += ((g_q.sigma * c_bar)/(SQRT_2_PI)) * (ERF(-mu_bar/(SQRT_2 * g_q.sigma)) - ERF((s - mu_bar)/(SQRT_2 * g_q.sigma)));
    }
    return _exp(T);
}
...
f32 simd_transmittance(const vec4f_t _o, const vec4f_t _n, const f32 s, const gaussians_t &gaussians)
{
    ...
    for (u64 i = 0; i < gaussians.gaussians.size(); i += SIMD_FLOATS)
    {
        ...
        const simd::Vec<simd::Float> c_bar = g_q.magnitude * SIMD_EXP(-((g_q.mu - o).dot(g_q.mu - o) - (mu_bar * mu_bar))/(simd::set1(2.f) * g_q.sigma * g_q.sigma));

        T += ((g_q.sigma * c_bar)/simd::set1(SQRT_2_PI)) * (SIMD_ERF(-mu_bar/(simd::set1(SQRT_2) * g_q.sigma)) - SIMD_ERF((simd::set1(s) - mu_bar)/(simd::set1(SQRT_2) * g_q.sigma)));
    }
...
simd::Vec<simd::Float> broadcast_transmittance(const simd_vec4f_t &o, const simd_vec4f_t &n, const simd::Vec<simd::Float> &s, const gaussians_t &gaussians)
{
    ...
    for (const gaussian_t &_G_q : gaussians.gaussians)
    {
        ...
        T += ((G_q.sigma * c_bar)/simd::set1(SQRT_2_PI)) * (SIMD_ERF(-mu_bar/(simd::set1(SQRT_2) * G_q.sigma)) - SIMD_ERF((s - mu_bar)/(simd::set1(SQRT_2) * G_q.sigma)));
    }
    return SIMD_EXP(T);
}
