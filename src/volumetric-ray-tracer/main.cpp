...
/// generated using spline interpolation with supports [-10.0 -9.0 -8.0 -7.0 -6.0 -5.0 -4.5 -4.0 -3.5 -3.0 -2.5 -2.0 -1.75 -1.5 -1.25 -1.0 -0.75 -0.5 -0.25 0.0]
static simd::Vec<simd::Float> simd_spline_exp(simd::Vec<simd::Float> x)
{
    simd::Vec<simd::Float> value = simd::set1(1.f);
    ...
    if (with_tests)
    {
        ...
        if (fork() == 0)
        {
            ...
            for (u64 i = 1; i < 100; ++i)
            {
                ...
                // BUG: Why does simd::getTimeSpec not work? Resutlt is always 0
                //      Using clock_gettime directly yields the same result as std::chrono...
                auto start_time = std::chrono::system_clock::now();
                for (u64 j = 0; j < SIMD_FLOATS * i; j+=SIMD_FLOATS)
                    simd::storeu(s + j, simd_spline_erf(simd::loadu(t + j)));
                auto end_time =  std::chrono::system_clock::now();
                i64 t_simd = std::chrono::duration_cast<std::chrono::nanoseconds>(end_time - start_time).count();
                
                start_time = std::chrono::system_clock::now();
                for (u64 j = 0; j < SIMD_FLOATS * i; j+=SIMD_FLOATS)
                    simd::storeu(s + j, simd_spline_erf_mirror(simd::loadu(t + j)));
                end_time =  std::chrono::system_clock::now();
                i64 t_simd_mirror = std::chrono::duration_cast<std::chrono::nanoseconds>(end_time - start_time).count();
                
                start_time = std::chrono::system_clock::now();
                for (u64 j = 0; j < SIMD_FLOATS * i; ++j)
                    s[j] = spline_erf(t[j]);
                end_time =  std::chrono::system_clock::now();
                i64 t_spline = std::chrono::duration_cast<std::chrono::nanoseconds>(end_time - start_time).count();
                
                start_time = std::chrono::system_clock::now();
                for (u64 j = 0; j < SIMD_FLOATS * i; ++j)
                    s[j] = std::erf(t[j]);
                end_time =  std::chrono::system_clock::now();
                i64 t_std = std::chrono::duration_cast<std::chrono::nanoseconds>(end_time - start_time).count();
