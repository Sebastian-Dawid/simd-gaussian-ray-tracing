...
template<decltype(simd_transmittance<>) Tr = simd_transmittance>
...
template<decltype(broadcast_transmittance<>) Tr = broadcast_transmittance>
vec4f_t simd_l_hat(const vec4f_t _o, const vec4f_t _n, const gaussians_t &gaussians)
{
    ...
    for (u64 i = 0; i < gaussians.gaussians.size(); i += SIMD_FLOATS)
    {
        ...
        for (i8 k = -4; k <= 0; ++k)
        {
            ...
            const simd::Vec<simd::Float> T = Tr(o, n, s, gaussians);
...
template<decltype(l_hat<>) Lhat = l_hat>
...
bool simd_render_image(const u32 width, const u32 height, u32 *image, const camera_t &cam, const vec4f_t &origin, const gaussians_t &gaussians, const bool &running = true)
{
    ...
    for (u64 i = 0; i < width * height; i += SIMD_FLOATS)
    {
        ...
        dir.normalize();
        const simd_vec4f_t color = broadcast_l_hat<Exp, Erf>(simd_origin, dir, gaussians);
        ...
        for (u64 tidx = 0; tidx < tiles.w * tiles.h; ++tidx)
        {
            ...
            std::function<void()> task = [img{tile_buffers[tidx]}, tidx, tile_width, tile_height, g, &tiles, &cam, &simd_origin] () {
                for (u64 _i = 0; _i < tile_width * tile_height; _i += SIMD_FLOATS)
                {
                    ...
                    dir.normalize();
                    simd_vec4f_t color = broadcast_l_hat<Exp, Erf>(simd_origin, dir, g);
