...
\begin{document}
    ...
    To get the final color of a pixel I need to determine the amount of light that reaches the camera from each point
    along a ray. For this purpose I determine the \gls{transmittance} $T$ of a point at distance $s$ along a ray from
    ...
    $\mathbf{x} = \mathbf{o} + s\mathbf{n}$. I assume the ambient \gls{radiance} is fixed as $L_e = 1$. As such I
    ...
    a compact interval $S_q = \{ \bar{\mu}_q + k\lambda_q | k \in K \subset \Z \}$. For my purposes it is sufficient
    ...
    \end{equation}
    \begin{equation}
        \hat{L}(\mathbf{o}, \mathbf{n}, \gamma) = \sum_{q} \mathbf{a}_q \sum_{s \in S_q} \lambda_qG_q(\mathbf{o} + s\mathbf{n})\sum_{i = 0}^W T^W_i(\mathbf{o}, \mathbf{n}, s, \gamma)
    \end{equation}
    ...
    Given this broadcast version of the \gls{transmittance} equation I can broadcast the \gls{radiance} equation to
    ...
    sums still iterate over every Gaussian $G_q \in \mathcal{G}$ individually.
