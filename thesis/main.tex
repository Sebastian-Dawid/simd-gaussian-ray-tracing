...
\begin{document}
    ...
    and graphics contexts, since its fully differentiable and its applicability in real time contexts.
    ...
    \end{equation}
    with $J$ as the jacobian of $P$ at $\mu$. \citeauthor{kerbl3Dgaussians} cite that \citeauthor{volume_splatting}
    also show that discarding the third row and column yields a 2D covariance matrix $\Sigma_{2D}$.
    ...
    \sqrt{2(\ln{(1)} - \ln{(\frac{1}{255})})} = 3.3$ is a sufficient approximation since $k(10) = 3.9$.
    ...
    \begin{equation}
        ...
        \label{eq:taylor}
    \end{equation}
    can give a good approximation of the error function around $x = 0$ when evaluated at $x_0 = 0$. For the
    approximation to be usable on the relevant interval of $[-2, 2]$ it needs to include at least ten terms of the
    series as seen in Figure~\ref{fig:taylor_erf}.
...
svml_exp(simd::Vec<simd::Float, 64> x);
{
    ...
    point format and the Taylor series of the exponential function. I provide wrapped versions of the approximation in
