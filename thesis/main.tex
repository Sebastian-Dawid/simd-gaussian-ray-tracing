...
simd_render_image<vcl_exp, simd_abramowitz_stegun_erf>
    ...
    All experiments were performed on a machine running Linux\footnote{Kernel version 6.10.9-arch1-1} with a Ryzen 9
    7950X CPU, which has 16 cores with hyperthreading and AVX512 support. For the duration of the experiments all cores
    of the CPU were locked to a frequency of $3.0$GHz. I locked the frequency using the userspace frequency scaling
    governor and \mintinline{sh}{cpupower}\footnote{The name of the tool appears to be distribution dependent.} utility.
...
gcc -march=native -dM -E - < /dev/null \
    ...
    the accuracy on the intervals relevant to the application to ray tracing (\ref{sec:expr_accuracy}).
    ...
    To benchmark the individual functions I measure the CPU cycles it takes to execute the function $N$ times. I average
    this result over $1000$ runs, which yields the average CPU cycle count for $N$ executions. I repeat this for increasing
    values for $N$. Finally, I write these measurements into a \mintinline{sh}{.csv} file along with the number of
    executions $N$.

    The benchmark is implemented as a macro shown in Listing~\ref{lst:benchmark_macro}. The code to benchmark
    (\mintinline{c++}{FUNC}) is enclosed by memory barriers to ensure the compiler does not move parts of it out of the
    enclosing \mintinline{c}{rdpmc_read} calls.

    The \mintinline{c++}{BENCHMARK} macro is used as shown in Listing~\ref{lst:benchmark_example}. Where \mintinline{c++}{t}
    is initialized with pseudorandom values generated with \mintinline{c}{drand48} and the results are written back to
    unique memory locations to ensure the calls are not optimized out by the compiler.
...
{                                           \
    ...
    {                                       \
        ...
        \end{minted}
        \caption{Benchmark Macro}
        \label{lst:benchmark_macro}
    \end{listing}

    \begin{listing}[t]
        \begin{minted}[linenos, frame=lines]{c++}
BENCHMARK(ctx, start, end, acc, for (u64 j = 0;
        j < SIMD_FLOATS * i; ++j)
        s_abramowitz[j] = abramowitz_stegun_erf(t[j]););
f32 ac_abramowitz = static_cast<f32>(acc)/ITER;

BENCHMARK(ctx, start, end, acc, for (u64 j = 0;
        j < SIMD_FLOATS * i; j+=SIMD_FLOATS)
        simd::storeu(s_simd_abramowitz + j,
        simd_abramowitz_stegun_erf(simd::loadu(t + j))););
f32 ac_simd_abramowitz = static_cast<f32>(acc)/ITER;
        \end{minted}
        \caption{Example usage of the \mintinline{c++}{BENCHMARK} macro.}
...
$ hpcviewer hpctoolkit-<binary>-database
    ...
    \begin{equation}
        \text{waste}(\text{cycles}, \text{instructions}) = 2 \cdot \text{cycles} - \text{instructions},
        \label{eq:vec_waste}
    \end{equation}
    ...
    Note that the two in the waste and efficiency formulas are based on the facts that most of the \gls{simd} operations
    have a throughput of $0.5$ which means that two can be executed per cycle.
    ...
    \begin{itemize}
        \item exp: \mintinline{c++}{approx::simd_fast_exp} is slightly faster than \mintinline{c++}{approx::vcl_exp},
            but a lot less accurate →  \mintinline{c++}{approx::vcl_exp} as default
        \item erf: \mintinline{c++}{approx::simd_abramowitz_stegun_erf} is significantly faster and accurate than the
            runner-up \mintinline{c++}{approx::simd_spline_erf_mirror} →  \mintinline{c++}{approx::simd_abramowitz_stegun_erf}
            as default.
    ...
    \begin{itemize}
        \item GCC better at sequential and innermost loop SIMD
        \item Clang Mode 8 w/ multithreading fastest overall
        \item SVML always the slowest
    \end{itemize}
