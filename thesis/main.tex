...
%@TODO: Should I keep citing the full name of papers upon introduction?
...
\begin{document}
    ...
    \par\noindent\makebox[8cm][l]{Sebastian Dawid}
    \cleardoublepage
...
% Sentence 7: PERSPECTIVES - what now?
    ...
    their \citeyear{kerbl3Dgaussians} paper \citetitle{kerbl3Dgaussians}\cite{kerbl3Dgaussians}. \citeauthor{kerbl3Dgaussians}
    ...
    %@TODO: Move this to the implementation section entirely since it is too implementation oriented?
...
Vec<f32, 32> mul(const Vec<f32, 32> &a,
    ...
    \end{minted}
    is the instantiation of the \mintinline{c++}{mul} function for 32 byte single precision floating point vectors.
    ...
    Note that \citeauthor{fast_exp} uses two 32-bit integers $i$ and $j$ in a \mintinline{c}{struct} to represent a
    ...
    %@TODO: Create figure to show this if there is enough time
    ...
    for this purpose instead. In my implementation I opted to use \mintinline{c++}{std::bit_cast} for the reinterpretation
...
f32 abramowitz_stegun_erf(f32 x)
{
    ...
    return sign * val;
        ...
        \end{minted}
        \caption{Implementation of approximation of the error function in Eq.~\refeq{eq:abrasteg}.}
...
tiles_t tile_gaussians(const f32 tw, const f32 th,
    ...
    can be translated into an algorithm directly. Note that the function returns a \mintinline{c++}{tiles_t} object. The
    ...
    After all possibly viable Gaussians have been checked, for the current tile, a \mintinline{c++}{gaussian_vec_t} object
...
simd_vec4f_t broadcast_radiance(const simd_vec4f_t o,
    ...
    \end{minted}
    as its signature and is, same as \mintinline{c++}{broadcast_transmittance}, identical to the sequential version,
    ...
    The two variants of the overloaded \mintinline{c++}{render_image} function do not allow for parallel processing of
...
bool render_image(const u32 width, const u32 height,
    ...
    The two variants of the overloaded \mintinline{c++}{simd_render_image} function process pixels in parallel, but do
    ...
    here are \mintinline{c++}{broadcast_radiance} and \mintinline{c++}{broadcast_transmittance}. Their signatures are
...
bool simd_render_image(const u32 width, const u32 height,
    ...
    library by passing the image generated by the rendering function to the \mintinline{c}{stbi_write_png} function along
...
gcc -march=native -dM -E - < /dev/null \
    ...
    The \mintinline{c++}{benchmark} template function is used as shown in Listing~\ref{lst:benchmark_example}. Where the variable
...
f32 ac_simd_abramowitz = static_cast<f32>(benchmark<1000,
    ...
    %@TODO: Move at least part of this to the appendix? Would I need a new appendix chapter for this?
    ...
    \mintinline{sh}{hpcviewer} allows filtering out nodes based on the name associated to the scope they represent. The
    ...
    generated by the compiler for the two instantiations of the \mintinline{c++}{radiance} template function.
    ...
    In the case of mode eight this is the \mintinline{c++}{broadcast_transmittance} function with a data cache miss rate
    ...
    \begin{table}[ht]
        ...
        \begin{tabular}{|c|c|c|}
            ...
            \mintinline{c++}{radiance}      & $1.57 \cdot 10^{11}$ ($98.26\%$) & $1.74\%$\\
            \mintinline{c++}{transmittance} & $1.73 \cdot 10^{14}$ ($95.57\%$) & $7.43\%$ \\
            \hline
        \end{tabular}
    ...
    \begin{table}[ht]
        ...
        \begin{tabular}{|c|c|c|}
            ...
            \mintinline{c++}{radiance}      & $0.22\%$ & $2.50\%$\\
            \mintinline{c++}{transmittance} & $0.02\%$ & $1.03\%$ \\
            \hline
        \end{tabular}
    ...
    \begin{table}[ht]
        ...
        \begin{tabular}{|c|c|c|}
            \hline
            Function                                     & Waste (\%)                       & Efficiency\\\hline
            \mintinline{c++}{radiance}                   & $1.62 \cdot 10^{11}$ ($84.84\%$) & $15.16\%$\\
            \mintinline{c++}{simd_transmittance}         & $9.06 \cdot 10^{12}$ ($53.68\%$) & $46.32\%$\\
            \mintinline{c++}{simd_abramowitz_stegun_erf} & $3.06 \cdot 10^{12}$ ($47.67\%$) & $52.33\%$\\
            \mintinline{c++}{vcl_exp}                    & $6.17 \cdot 10^{12}$ ($66.33\%$) & $33.67\%$\\
            \hline
        \end{tabular}
    ...
    \begin{table}[ht]
        ...
        \begin{tabular}{|c|c|c|}
            \hline
            Function                                     & icmr     & dcmr\\\hline
            \mintinline{c++}{radiance}                   & $0.58\%$ & $4.96\%$\\
            \mintinline{c++}{simd_transmittance}         & $0.14\%$ & $4.98\%$\\
            \mintinline{c++}{simd_abramowitz_stegun_erf} & $0.13\%$ & $3.42\%$\\
            \mintinline{c++}{vcl_exp}                    & $0.18\%$ & $5.59\%$\\
            \hline
        \end{tabular}
    ...
    \begin{table}[ht]
        ...
        \begin{tabular}{|c|c|c|}
            \hline
            Function                                     & Waste (\%)                       & Efficiency\\\hline
            \mintinline{c++}{simd_radiance}              & $8.65 \cdot 10^{9}$ ($57.63\%$) & $42.80\%$\\
            \mintinline{c++}{broadcast_transmittance}    & $4.29 \cdot 10^{12}$ ($42.80\%$) & $57.63\%$\\
            \mintinline{c++}{simd_abramowitz_stegun_erf} & $4.37 \cdot 10^{12}$ ($40.12\%$) & $59.88\%$\\
            \mintinline{c++}{vcl_exp}                    & $2.47 \cdot 10^{12}$ ($34.33\%$) & $65.67\%$\\
            \hline
        \end{tabular}
    ...
    \begin{table}[ht]
        ...
        \begin{tabular}{|c|c|c|}
            \hline
            Function                                     & icmr     & dcmr\\\hline
            \mintinline{c++}{simd_radiance}              & $0.10\%$ & $21.20\%$\\
            \mintinline{c++}{broadcast_transmittance}    & $0.27\%$ & $0.96\%$\\
            \mintinline{c++}{simd_abramowitz_stegun_erf} & $0.16\%$ & $0.37\%$\\
            \mintinline{c++}{vcl_exp}                    & $0.08\%$ & $0.50\%$\\
            \hline
        \end{tabular}
    ...
    \begin{table}[ht]
        ...
        \begin{tabular}{|c|c|c|}
            \hline
            Function                                     & Waste (\%)                       & Efficiency\\\hline
            \mintinline{c++}{broadcast_radiance}         & $6.44 \cdot 10^{9}$ ($50.65\%$)  & $49.35\%$\\
            \mintinline{c++}{broadcast_transmittance}    & $4.29 \cdot 10^{12}$ ($42.86\%$) & $57.14\%$\\
            \mintinline{c++}{simd_abramowitz_stegun_erf} & $4.37 \cdot 10^{12}$ ($40.28\%$) & $59.72\%$\\
            \mintinline{c++}{vcl_exp}                    & $2.55 \cdot 10^{12}$ ($35.14\%$) & $64.86\%$\\
            \hline
        \end{tabular}
    ...
    \begin{table}[ht]
        ...
        \begin{tabular}{|c|c|c|}
            \hline
            Function                                     & icmr     & dcmr\\\hline
            \mintinline{c++}{broadcast_radiance}         & $0.06\%$ & $11.73\%$\\
            \mintinline{c++}{broadcast_transmittance}    & $0.20\%$ & $1.04\%$\\
            \mintinline{c++}{simd_abramowitz_stegun_erf} & $0.14\%$ & $0.51\%$\\
            \mintinline{c++}{vcl_exp}                    & $0.04\%$ & $0.48\%$\\
            \hline
        \end{tabular}
    ...
    \begin{figure}[ht]
        ...
        \caption{\mintinline{sh}{hpcviewer} Filters Used}
