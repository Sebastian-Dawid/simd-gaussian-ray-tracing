...
function load_data()
    ...
    erf_approx = plot(x, y_spline, dpi=300, label="spline", xlabel=L"x", ylabel=L"$y$", legend=:topleft)
    ...
    exp_approx = plot(x, y_spline, dpi=300, label="spline", xlabel=L"x", ylabel=L"$y$", legend=:topleft)
    ...
    exp_err = plot(x, abs.((y - y_spline) ./ x), dpi=300, label="spline", xlabel=L"x", ylabel="err", legend=:topleft)
